...
@app.cls(gpu=gpu.A10G(), container_idle_timeout=240, image=flux_image)
class Model:
    @build()
    def build(self):
        import os
        ...
        subprocess.run("aria2c --console-log-level=error -c -x 16 -s 16 -k 1M https://huggingface.co/camenduru/FLUX.1-dev/resolve/main/flux1-dev-fp8.safetensors -d /TotoroUI/models/unet -o flux1-dev-fp8.safetensors", shell=True,)
    ...
    def _inference(self, prompt):
        ...
        with torch.inference_mode():
            ...
            sigmas = self.BasicScheduler.get_sigmas(self.unet, scheduler, steps, 1.0)[0]
